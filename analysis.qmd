...
## Education Level Change Over the Years {toc-text="Graph 1"}
...
### **Illiterates** {toc-text="Illiterates"}
...
illiterateoveryears <- data |>
  filter(!is.na(Region)) |>
  ggplot(aes(`General Population` / 10^6,`General Illiterate Population` / 10^6,color = Region)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10() +
  geom_text(aes(label = `Province code`), hjust = 0.5, vjust = -0.5, size = 2) +
  labs(
    title = "Percentage of Illiteracy Relative to the General Population in Year :{frame_time}",
    x= "General Population in Millions" ,
    y= "General Illiterate Population in Millions"
  ) +
  transition_time(as.integer(Year))
anim_save("illiterateoveryears.gif", animation = illiterateoveryears)
```
...
![](Illiteracy Change Rates by Region 08-23.png)
```{r message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Show the code"
